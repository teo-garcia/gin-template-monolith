...
`.golangci.yml` is generated by `make lint-sync` and gitignored. Put
repository-specific lint rules in `golangci.overrides.yml`; the generator
deep-merges that overlay onto the shared baseline. `make check` also obtains its
coverage flags and reporters from `gotest-config-shared`, so local and CI checks
exercise the same shared contract.
